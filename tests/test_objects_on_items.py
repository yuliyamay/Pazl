...
def test_verify_objects(browser):
    ...
    backpack_image_exists = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_ITEM_IMAGE
    )
    assert backpack_image_exists, "Backpack image is not displayed"  # works
    backpack_name_exists = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_ITEM_NAME
    )
    assert backpack_name_exists, "Backpack name is not displayed"  # works
    backpack_description_exists = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_PRODUCT_DESCRIPTION
    )
    assert backpack_description_exists, "Backpack description is not displayed"  # works
    backpack_price_exists = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_PRICE
    )
    assert backpack_price_exists, "Backpack price is not displayed"  # works
    backpack_add_to_cart = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_ADD_TO_CART_BUTTON
    )
    assert backpack_add_to_cart, "Backpack add to cart is not displayed"  # works
    back_to_product_button_exists = driver.element_is_present(
        *InventoryPageLocators.BACKPACK_BACK_BUTTON
    )
